# Understudy — 세션 핸드오프 메모 (2026-09-03, 첨부 감사 대응 갱신)

다른 세션/에이전트가 이 프로젝트를 이어받기 위한 단일 문서.
저장소: github.com/wilcoco/nvidia (public, MIT)

> **현재 상태 — 첨부 `0282f98` 감사 대응:** 작업 시작 기준은 `6fb541d`였으며 코드 검증을 마쳤을 때 HEAD와 원격 main은 다른 동시 작업이 16:36에 커밋·푸시한 `86d46b4`였다. 이 커밋에 이번 코드·테스트 개선도 포함되어 있다. 기존 온보딩·양식 개선을 보존하면서 완료/서명 기록의 DB 잠금 내 불변성, 자동 리뷰 생성 중복 방지와 응답 유실 복구, 현재 탭만 비우는 새 업무 시작, 모바일 오류·폭·역할 안내, 문서·큐시트를 보완했다. 실제 두 탭 E2E에서 검토자 탭이 예전 성공을 다시 저장하는 경합을 추가 발견해 수정했고, 저장된 설계와 실제 Run의 화면도 구분했다. **타입 검사·빌드·메모리/PostgreSQL 자동 테스트 42개 통과**, 실제 HTTP/DB 전달 장애 30조건 포함. 로컬 18991에서 반복 실패 3회, 늦은 실패 교정과 리뷰 취소, 별도 탭 승인, 새 설계/버전/실행 후 이전 완료 DB 행 무변경을 확인했다. [첨부 감사 조치·검증 결과](AUDIT_FIXES_2026-09-03.md)가 최신 근거다. **사용자의 후속 “푸시해” 요청으로 이번 최종 감사 보고서·핸드오프 변경의 커밋·푸시가 승인됐다. 이전 푸시 보류는 이번 요청으로 해제됐다.** 라이브 프런트 번들에 `86d46b4`가 포함된 것은 확인했지만 배포판 전체 E2E 재검은 하지 않았다. 전역 reset API는 비활성화했으므로 아래 과거 초기화·정리 계획을 실행하지 않는다. 검증 후 현재 탭은 빈 업무 시작 화면이며, 로컬 QA 자료는 보존했다. 아래 내용은 시간순 과거 이력으로 읽을 것.

> **이전 로컬 상태 (2026-09-03, 온보딩 개선 시점):** 당시 HEAD/origin의 기준은 `ebc93d1`이었다. 그 위에서 첫 방문 소개·사용법의 WebMCP 웹 반응, 업무 저장 직후 첫 질문과 답 영속화, 단계별 숫자/드롭다운 양식·귀속/값 검증, 선택값에 따른 분기, 다른 탭의 결재 후 현재 실행 자동 갱신을 수정했다. **커밋·푸시·배포하지 않았다. 아래 과거 기록의 푸시 지시는 현재 승인으로 해석하지 않는다.** [업무 발견·양식·분기·협업 갱신 검증](ONBOARDING_AND_FIELDS_2026-09-03.md)을 먼저 읽을 것. 타입 검사·빌드와 메모리/격리 PostgreSQL 테스트 33개 통과. 로컬 18991에서 Pickup 경로 run #3은 별도 탭 결재→자동 인계→완료→새로고침 복원까지 검증했다. Courier 경로 run #4는 다른 분기에 따른 Park의 업무가 나타나는 상태로 남겼다. 검증자는 QA 자료를 직접 구성했으며 사용자 연구 증거로 취급하지 않는다. **데스크톱 WebMCP가 작성·수정·분기 처리, 모바일은 담당 업무 입력·결재**라는 범위를 유지한다. 최신 확인 라이브 빌드는 `ebc93d1`이며 이 로컬 수정은 아직 반영하지 않았다.
...
- 과거 촬영 전 청소 계획은 폐기했다. 공유 초기화 API는 비활성화 상태이며,
  감사 fixture와 기존 자료를 보존한다. 새 업무는 현재 탭에서만 시작한다.
...
- **사이트 자체 LLM 키 없음**: 페이지는 도구를 제공하고 방문자의 WebMCP 에이전트가 추론한다.
  방문자 클라이언트의 이용 조건·비용은 별도이며 확인된 검증 클라이언트는 Codex 내장 브라우저다.
...
- **결정(decision) 해소는 에이전트 전용**: resolve_decision이 곧 증거 검증
  관문(evidence_conflict)을 사용한다. 현재 모바일 제품 범위는 담당 업무 입력·리뷰다.
...
2. **QA 자료 보존**: 과거 공유 초기화 계획은 폐기. 촬영 시작은 설정의 현재 탭 새 업무
   시작을 사용한다. 시연 플레이북과 과거 실행·감사 기록을 삭제하지 않는다.
...
- 공유 데모를 전역 초기화하지 않는다. 현재 탭의 미저장 초안만 정리한다.
