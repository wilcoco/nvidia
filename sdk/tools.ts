...
export const tools: ToolDef[] = [
  {
    ...
    description:
      'Start here for introductory or help questions. Interpret intent, not exact wording: "What is this?", "What does this service do?", "Who is it for?", and "What can I do here?" request guide_topic:"overview". "How do I use it?" and "Where do I start?" request guide_topic:"usage". Set show_guide:true so the page responds with the relevant guide, and set guide_language to the visitor\'s language. For routine context reads, omit show_guide so the page is not interrupted. Returns a plain-language introduction, creating vs running guidance and current context. Answer the actual question in the visitor’s language, then offer one relevant next step. No records, drafts or runs are changed; opening help preserves their work. Visitors do not need tool names or a magic phrase. Also lists available actions and process status for continuing with WebMCP.',
  ...
  },
  {
    name: 'get_map_gaps',
    description:
      "The adaptive interview agenda: structural gaps plus one human-selected judgment point at a time. The highest-value judgment point is active by default; the agent must not expand to another point unless the human clicks ‘Explore another judgment point’ in the panel. Within the active point, elicitation progresses one move at a time: a concrete incident, observable cues, the tempting novice mistake, boundary/exception, then failure/recovery. Each gap includes priority, method and the exact resolves_gap key to pass to ask_user. Write one short question YOURSELF in this app's domain language; fallback_question is only for an unknown domain and ask_user rejects it verbatim. When answered, the page automatically preserves the raw question, answer, timestamp and disposition on that step. Read it back with get_process_map, tell the human what you understood, and encode fields, thresholds, branches or recovery only after their answer supports them. Never invent a missing slot or treat raw prose as executable criteria.",
    inputSchema: schema(),
    execute: async () => {
